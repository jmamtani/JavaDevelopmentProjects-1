...
public final class StringReassembly {
    ...
    public static void addToSetAvoidingSubstrings(Set<String> strSet,
        ...
        boolean endCondition = false; //initializing the boolean as false
        while (strSet.size() > 0) { //while loop until the end of string is reached
            String x = strSet.removeAny();
            if (x.indexOf(str) >= 0) { //condition when the index of str is greater than 0
                endCondition = true;
            }
            if (!(str.indexOf(x) >= 0)) {
                temp.add(x); //for when the index of stress is !0 add the index to temp
            }

        }
        if (!endCondition) { //add the str to the temp set
            temp.add(str);
        }
        strSet.add(temp); //adding the temp set to the final str set
    }
